...
export const api = createApi({
  ...
  endpoints: (build) => ({
    ...
    createRecipeIngredient: build.mutation<
      any,
      { recipe: number; ingredient_data: { name: string }; quantity?: number; unit?: string }
    >({
...
getShoppingLists: build.query<any, void>({
  query: () => "shopping-lists/",
  providesTags: ["ShoppingList"],
}),
createShoppingList: build.mutation<any, { title: string; items?: any[] }>({
  query: (body) => ({ url: "shopping-lists/", method: "POST", body }),
  invalidatesTags: ["ShoppingList"],
}),
updateShoppingList: build.mutation<any, { id: number; patch: Partial<any> }>({
  query: ({ id, patch }) => ({ url: `shopping-lists/${id}/`, method: "PATCH", body: patch }),
  invalidatesTags: ["ShoppingList"],
}),
deleteShoppingList: build.mutation<{ success: boolean }, number>({
  query: (id) => ({ url: `shopping-lists/${id}/`, method: "DELETE" }),
  invalidatesTags: ["ShoppingList"],
}),

// --- SHOPPING ITEMS ---
getShoppingItems: build.query<any, { shopping_list?: number } | void>({
  query: (params) => {
    const q = params?.shopping_list ? `?shopping_list=${params.shopping_list}` : "";
    return `shopping-items/${q}`;
  },
  providesTags: ["ShoppingList"], // change to your preferred tag if you want item-level granularity
}),

createShoppingItem: build.mutation<any, {
  shopping_list: number;
  ingredient: number;
  quantity?: number;
  unit?: string;
}>({
  query: (body) => ({ url: "shopping-items/", method: "POST", body }),
  invalidatesTags: ["ShoppingList"],
}),
updateShoppingItem: build.mutation<any, { id: number; patch: Partial<any> }>({
  query: ({ id, patch }) => ({ url: `shopping-items/${id}/`, method: "PATCH", body: patch }),
  invalidatesTags: ["ShoppingList"],
}),
deleteShoppingItem: build.mutation<{ success: boolean }, number>({
  query: (id) => ({ url: `shopping-items/${id}/`, method: "DELETE" }),
  invalidatesTags: ["ShoppingList"],
}),
  })
});
